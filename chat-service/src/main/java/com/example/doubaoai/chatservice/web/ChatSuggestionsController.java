    ...
    public ResponseEntity<SuggestionsResponse> suggestions(@Valid @RequestBody SuggestionsRequest req) {
        return store.find(req.sessionId())
                .map(session -> {
                    ...
                    String system = """
                            你是「猜你想问」助手：根据上面「用户最近提问」与「助手最近回复」，推测用户接下来**还想了解什么**（用户视角的后续问题），不是助手追问用户、也不是让用户反问助手。
                            生成恰好3条**用户可能会继续问**的问题；每条必须与上文主题直接相关，禁止泛泛的「你好」「谢谢」「介绍一下」等无关句。
                            每条要自然、通顺，像用户自己会打出来的问法。
